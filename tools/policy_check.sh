...
# ── CHECK 14: No hand-rolled wait-loop / self-matching pgrep in plugin prose ─
#
# STRUCTURAL PREVENTION for issue #279: runtime-loaded plugin guidance must not
# PRESCRIBE the two forbidden wait-loop anti-patterns that the Bash Command
# Discipline rule (definitions.md) forbids:
#   (a) a hand-rolled `until ! pgrep` poll-loop used to wait for a long-running
#       command to finish (the only sanctioned wait primitive is a background
#       task completion notification or a Monitor), and
#   (b) a self-matching `pgrep -f "<token>"` whose quoted token also appears as
...
# `## Bash Command Discipline` no-hand-rolled-wait-loop / no-self-match bullets,
# which contain `until`, `pgrep`, and `pgrep -f "tools/validate.sh"`). Mirroring
# CHECK 1's self-exemption of its own rule text, any definitions.md line carrying
# a rule-anchor substring (`the only sanctioned wait primitive` or `self-match`)
# is skipped so the doctrine that DEFINES the prohibition is never flagged by it.

echo ''
echo '=== CHECK 14: No hand-rolled wait-loop / self-matching pgrep in plugin prose ==='

# Rule-anchor substrings that mark the definitions.md bullets which legitimately
# quote the forbidden constructs. A line in definitions.md containing either is
# the rule definition itself, not a prescription, and is exempt.
CHECK14_DEFINITIONS_REL='plugin/governance/definitions.md'
CHECK14_RULE_ANCHOR='the only sanctioned wait primitive|self-match'

check14_found=false
while IFS= read -r -d '' md_file; do
    # Compute the repo-relative path once so the definitions.md self-exemption
    # is a path comparison, not a basename guess.
    md_rel="$md_file"
    if [[ "$md_file" == "$REPO_ROOT"* ]]; then
        md_rel="${md_file#"$REPO_ROOT"/}"
    fi
    md_rel="${md_rel//\\//}"
    is_definitions=false
    if [[ "$md_rel" == "$CHECK14_DEFINITIONS_REL" ]]; then
        is_definitions=true
    fi

    # One-pass awk state machine emits surviving BODY lines as "line_num<TAB>line",
    # excluding YAML frontmatter, fenced code blocks, and markdown table rows
    # (identical scoping to CHECK 11).
    while IFS=$'\t' read -r line_num textline; do
        [[ -z "$line_num" ]] && continue
        ...
        # SELF-EXEMPTION: definitions.md rule-definition bullets quote these
        # anti-patterns as the thing being forbidden — never flag them.
        if [[ "$is_definitions" == true ]] && echo "$textline" | grep -qE "$CHECK14_RULE_ANCHOR"; then
            continue
        fi

        # Pattern (a): a hand-rolled `until ! pgrep` poll-loop.
        if echo "$textline" | grep -qE 'until[[:space:]]+!?[[:space:]]*pgrep\b'; then
            check14_found=true
        ...
        # Pattern (b): a self-matching `pgrep -f "<token>"` where the quoted
        # token ALSO appears as an invoked path elsewhere on the same line. The
        # quoted token is captured, then sought again outside its own quotes; a
        # second occurrence is the self-match the rule forbids.
        pgrep_token="$(echo "$textline" | grep -oP "pgrep\s+-f\s+\"\K[^\"]+" | head -n1 || true)"
        if [[ -n "$pgrep_token" ]]; then
...
if [[ "$check14_found" == false ]]; then
    echo '[PASS] Check 14: No hand-rolled wait-loop / self-matching pgrep prescribed in plugin prose'
    CHECKS_PASSED=$((CHECKS_PASSED + 1))
else
    CHECKS_FAILED=$((CHECKS_FAILED + 1))
fi
