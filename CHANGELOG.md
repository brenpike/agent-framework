...
- ADR-0010 records a known gap: the `copilot` review provider has no `Bash(copilot *)` pre-approval in `hivemind:adaptation-cycle`'s `allowed-tools`, so a copilot review currently hits an interactive permission prompt.
