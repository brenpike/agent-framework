...
- **Manifest field extractor is now block-scalar-body-aware, closing a description-text → strain-structure override (P1).** `_shared/manifest.sh` previously matched any `key:` line at any indentation, so a strain's untrusted `description: |` body — issue-sourced free text reproduced verbatim — could embed counterfeit `status:`, `worktree_path:`, `branch:`, `tmux_session:`, nested `run.suggested_ledger:`, or an injected `- name:` strain entry, and the extractor would return the attacker's value before the genuine field (falsifying dashboard observables and redirecting the read-side ledger projector to an attacker-chosen worktree while still passing the token allowlist). Both `hivemind_manifest_strain_names` and `hivemind_manifest_field` now track block-scalar bodies by indentation: every line deeper than an opening `key: |`/`key: |-` is treated as inert body and never matched as structure, so description content can no longer override genuine strain fields. Regression-covered by a hostile-description fixture (`tests/brood/manifest-v2-hostile-desc.yaml`) in `tools/test_shared_libs.sh`.
