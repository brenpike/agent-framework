...
- **`brood-status` now distinguishes a present-but-unreadable manifest from an empty or absent brood, closing a monitoring blind spot.** The read-side projection helper (`brood-status-project.sh`) swallowed `jq` parse errors and projected a torn / truncated / invalid-JSON manifest as zero strains, so corruption was indistinguishable from a genuinely empty brood and hid live children from the dashboard. After pre-flight, the helper now probes manifest parseability (`jq empty`); a PRESENT-but-unparseable manifest emits a distinct `MANIFEST_UNREADABLE` sentinel line and exits 2, while a VALID empty manifest (`{"strains":[]}`) still exits 0 with no strain lines. The `brood-status` navigator renders a prominent corruption warning on the sentinel rather than reporting "No active brood found." The write-side liveness guard's fail-open-on-malformed behavior is unchanged (a corrupt manifest must not wedge spawning). (Codex #172 read-side finding.)
