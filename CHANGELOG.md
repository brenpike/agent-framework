...
- `hivemind:decision-report` ledger READ is now likewise routed through the engine instead of a raw `jq` flatten in the skill body. The skill invokes `decision-report.sh --read-decisions <run_id>`, which derives the ledger from `run_id` + the git root and routes the read through the shared `hivemind_open_ledger` guard (depth-complete ancestor guard + runs-dir canonical prefix + `hivemind_assert_ledger_contained` leaf guard + existence/JSON-validity + `run.id` coherence) BEFORE any read, then emits the flattened `[.events[].outputs.decisions[]?]` decision list. This closes the READ-oracle twin of the write escape — a committed symlinked `<run_id>` dir or `state.json` leaf can no longer redirect the ledger read outside the checkout (the skill's raw `jq`/Read on the caller-derived `<run_id>` path fired before any check; an inline check was unsound). The `Bash(jq *)` grant is dropped from the skill's `allowed-tools` (no remaining procedure step needs raw `jq`).
