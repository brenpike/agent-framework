...
- github-review-loop poll: base-10-coerce `MAX_WATCH_SECONDS` and `POLL_INTERVAL_SECONDS` before arithmetic — leading-zero values (e.g. `08`/`09`) no longer trigger an octal arithmetic error that aborted the watch under `set -u`, and `060` is 60s (not 48s).
- github-review-loop poll: paginate `statusCheckRollup.contexts` when computing the check-identity fingerprint — check-set/state changes beyond the first 100 contexts are no longer missed (previously could skip waking the reviewer on very large PRs).
