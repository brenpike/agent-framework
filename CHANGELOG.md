...
- overlord-inline post-merge decision report: on a subsequent session start, the overlord's Resume-On-Start scan derives any run whose PR has merged/closed and whose journal carries ≥1 Tier-B auto decision, reads that run's OWN `decisions[]` from the ledger it owns, invokes the render-only `hivemind:decision-report` skill to narrate the auto-decisions in the consumer project's ubiquitous language, and persists the rendered narrative next to the run's `state.json` (the report file's existence is the idempotency marker). Additive free-form ledger conventions only — `run.status` enum, `schema_version`, and existing `event.outputs`/`artifacts` fields are unchanged (additions are backward-compatible).
