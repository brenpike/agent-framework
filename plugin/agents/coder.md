...
- When the delegation uses a `Step:`-omitting Bypass Allowlist code (`TRIVIAL_CHANGE` or `SINGLE_STEP_TASK`) and the task externalizes any evidence per Progressive Evidence Loading, list each externalized `EVD-NNN` anchor in the optional `Evidence refs:` line of the worker report per `${CLAUDE_PLUGIN_ROOT}/governance/communication-policy.md` (Shared Worker Report Contract). These bypasses omit `Step delta:`, so `Evidence refs:` is the only slot that records the anchor. `NO_PRIOR_PHASE` does not omit `Step:`/`Step delta:` (per `${CLAUDE_PLUGIN_ROOT}/governance/context-management-policy.md` (Bypass Allowlist)) — for first-phase multi-phase work, keep `Step: STEP-NNN` and record `EVD-NNN` anchors in the `Evidence:` field of the `Step delta:` section as usual.
