...
3. **Process events:** On each Monitor event:
   ...
   - Codex approval (`CODEX_APPROVED=` line): if the SAME poll has zero unresolved non-self actionable lines (no `THREAD=`, `COMMENT=`, `REVIEW=`, or `CHECK_FAIL=` lines), signal Monitor stop and return `clean`. If actionable items coexist, process them and KEEP WATCHING — do NOT terminate on approval. Use only the latest poll's approval state; a stale prior approval must not short-circuit later pushback.
