...
Likewise, never emit decorative or scaffolding shell output via Bash: no section-banner echos (`echo "=== X ==="`, `echo "---HEAD---"`), no progress/status narration (`echo "plugin.json OK"`, `echo "done"`), and no terse status tokens (`echo "JSON valid"`). Do not frame command output with echo separators — rely on the tool's own output. They produce no coordination value and only trigger permission prompts. (Load-bearing `printf` routing-data emissions required by pipeline skills, e.g. `printf 'branch: ...'`, are exempt; only DECORATIVE/NARRATION echo/printf is forbidden.)
