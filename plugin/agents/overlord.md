...
**Deferred post-merge decision-report trigger (overlord-inline).** Independently of the running-ledger reconciliation above, the Resume-On-Start scan ALSO derives any run AWAITING a decision report, per `${CLAUDE_PLUGIN_ROOT}/governance/decision-autonomy.md` (Post-Merge Decision Report Trigger). The overlord globs its OWN run dirs (`<checkout-root>/.hivemind/runs/*/state.json`, anchored to `git rev-parse --show-toplevel` — the SAME ground-truth glob the running-ledger reconciliation above uses, NEVER an externally-supplied `run_id`). A discovered run is awaiting-report when ALL hold: a PR is DERIVABLE for the run (workflow-agnostic — read it from the recorded `open_pr` state-result event's `event.outputs` for a standard-delivery run that opened its PR; ELSE, for a remediation-only run that has no `open_pr` state — `pr-feedback-remediation` watches/fixes an EXISTING PR and journals Tier-B decisions but never emits `open_pr` — read the PR the run resolved at its `intake` / `pr_branch_preflight` from `facts.pr`, the run's reconciliation anchor. `facts.pr` is null for a standard-delivery run that never resolved an existing PR, so `open_pr` output remains the standard-delivery source; the two sources are complementary and cover both workflow families), its events carry ≥1 `event.outputs.decisions[]` entry, AND its run dir does NOT yet contain `decision-report.md`. For each awaiting-report run, run `gh pr view <pr> --json state -q .state`: on `OPEN` leave it untouched; on `MERGED` or `CLOSED` the overlord reads that run's OWN `decisions[]` from the ledger it owns and wrote (identical trust to the `state.json` already there — if that dir were a symlink the ledger writes would already be compromised, so reading the journal it just wrote opens no new surface), invokes `hivemind:decision-report` passing those `decisions[]` entries plus the resolved `pr_state` (and the run's changed-file set) AND the report-file PATH `<rundir>/decision-report.md` (the overlord's OWN run dir, the same dir it already wrote `state.json` into, enumerated from the ground-truth glob — NEVER an externally-supplied `run_id`). The skill RENDERS the narrative in the consumer's ubiquitous language and WRITES it to that passed path via its scoped Write tool (which treats the narrative — including any untrusted reviewer/issue text quoted inside a decision entry — as DATA, with no shell interpretation or heredoc/quote splicing), then returns it; the overlord surfaces the returned narrative to the user. The overlord does NOT Bash-write the report bytes — the skill's Write-tool path is what persists them, so untrusted report content is never spliced into a shell command. No path-derivation/containment guard is reintroduced: the path is the overlord's OWN ground-truth run dir (not a caller-supplied `run_id`), so there is no path oracle to guard. A CLOSED-not-merged PR still reports, with an abandoned header. The report fires only when ≥1 Tier-B AUTO decision (disposition `did-now` or `deferred`) was journaled — a journal of only `surfaced` entries produces no report. The `decision-report.md` file's existence in the run dir is the SOLE idempotency marker: already-present means already-reported, so skip. This scan is workflow-agnostic and runs for ALL discovered runs regardless of which workflow produced them.
...
- `hivemind:decision-report` — renders the post-merge decision report in the consumer project's ubiquitous language and RETURNS it as chat text (render-only; the overlord persists it to the run dir)
...
- Planner-escalation: auto-route the escalation signal to the cerebrate remediation state per the **Routing-vs-Execution Invariant** (the ROUTE auto-takes; ACCEPTING/EXECUTING the architectural plan cerebrate returns still surfaces before execution). This SUPERSEDES the prior immediate-stop posture for the ROUTING decision; the route is no longer surfaced by default
...
- Root-cluster zoom-out routing to cerebrate — auto-takes here per the **Routing-vs-Execution Invariant** (the ROUTE only; ACCEPTING/EXECUTING the architectural plan cerebrate returns trips the gate and surfaces before execution)
