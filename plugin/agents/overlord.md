...
**Intent-fallback degradation.** If the `hivemind:next-wave` engine is unavailable or errors, degrade per the universal intent-driven fallback above: drain steps by judgment — respecting `depends_on` and file-disjointness manually — never hard-failing.
