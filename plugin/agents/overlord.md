...
RUN-OWNERSHIP-01: a run ledger is owned and mutated only by the overlord instance whose worktree contains it, enforced by worktree isolation. The hatchery reports strain status from external observables + manifest static fields and NEVER mutates child ledgers (child-ledger workflow-state reporting is deferred to issue #161); children NEVER mutate the hatchery manifest or hatchery ledger. The brood manifest is the sole shared artifact and only the hatchery writes it. The manifest is a registry/coordination artifact, not the source of truth for child workflow state — that lives in each child's run ledger. Full brood mechanics live in `hivemind:spawn-brood` and `hivemind:brood-status`; the manifest-v2 / child-task-metadata bridge and status-derivation priority are in `${CLAUDE_PLUGIN_ROOT}/references/brood-ledger-model.md`.
