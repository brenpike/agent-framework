...
- `agent-framework:address-github-pr-feedback`: two-mode invocation for one-time PR feedback where the user request does not contain `watch`, `monitor`, `wait`, `poll`, or `loop`. Used for Codex, human, and bot comment fixes alike. PR identification is the skill's responsibility — pass the user-named PR number if any, otherwise pass the current branch and let the skill resolve. Invocation pattern: (1) invoke with `mode: classify` (default) — skill fetches candidates, injection-scans, classifies, and returns a `Classification` + `Routing` recommendation without applying fixes; (2) orchestrator delegates the fix to the recommended framework agent (`coder`, `designer`, or `planner`) via the Agent tool; (3) invoke again with `mode: post-fix`, passing `fix_sha`, `validated`, `thread_id`, `target_comment_id`, `classification`, `severity_category`, and `pr_number` — skill posts the fix-SHA reply and resolves the thread. The skill does not delegate to framework agents.
...
- `agent-framework:review-loop-controller`: `base`, `working_branch`, `trunk`, `claude_mem`, `mode` (`iterate` | `continue` | `finalize`). Optional: `max_iterations` (integer, default `10`) — pass when re-invoking after `max-iterations-reached`. Optional: `fix_results` (list of `{finding_id, fix_sha, validated}`) — pass on `continue` invocations after fixes have been applied.
...
13a. **Pre-PR local review loop.** After validation and before pushing or opening a PR, drive the local review loop externally when ALL of the following are true: (a) the user has not opted out of local review (task input does not contain "skip review", "no review", or "skip local review"); (b) codex-plugin-cc availability is unknown or confirmed present (the skill itself detects availability).
   ...
   a. Invoke `agent-framework:review-loop-controller` via the Skill tool with `mode: iterate`, `base` (resolved trunk), `working_branch`, `trunk`, and `claude_mem`.
   ...
   b. On each iteration result:
      ...
      - If `Exit-reason: none` (iteration complete, findings returned): for each finding with `routing: coder`, delegate fix to `agent-framework:coder` via the Agent tool. For each finding with `routing: designer`, delegate to `agent-framework:designer` via the Agent tool. For each finding with `routing: planner`, delegate to `agent-framework:planner` via the Agent tool for a remediation plan, then delegate that plan to `agent-framework:coder` via the Agent tool for implementation (planner is read-only and cannot commit fixes). After all fixes are applied, invoke `agent-framework:checkpoint-commit` via the Skill tool. Then re-invoke the controller with `mode: continue`, passing `fix_results` (list of `{finding_id, fix_sha, validated}`) in addition to the usual `base`, `working_branch`, `trunk`, `claude_mem`.
