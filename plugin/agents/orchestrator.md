...
- when the delegation included a `Step: STEP-NNN` field and the worker's report includes a `Step delta:` section: extract that section and hold it in memory. Do not store or delegate yet — contradiction detection and reconstruction test must pass first (see below)
...
- **Store step-delta** only after both contradiction detection and reconstruction test pass: store the extracted step-delta as a claude-mem observation (when installed per `${CLAUDE_PLUGIN_ROOT}/governance/context-management-policy.md` (claude-mem Detection)) or write to `.agent-framework/handoffs/STEP-NNN.md`.
- **Delegate next phase** with only the compact step-delta context, not the full prior phase report or tool outputs.
