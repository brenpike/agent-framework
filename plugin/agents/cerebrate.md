...
- Prefer the already-granted Read / Glob / Grep tools and the concrete read-only git tools listed in frontmatter over Bash. Do not chain compound Bash pipelines (`a | b | c`) for inspection — the granted tools cover the same work directly. Never emit decorative or scaffolding shell output: no section-banner echos (`echo "=== X ==="`, `echo "---HEAD---"`), no progress/status narration (`echo "plugin.json OK"`, `echo "done"`), and no terse status tokens (`echo "JSON valid"`). Such output is noise that adds no analytic value — rely on the tool's own output instead of framing it with echo separators. (Load-bearing `printf` routing-data emissions required by pipeline skills, e.g. `printf 'branch: ...'`, are exempt; only DECORATIVE/NARRATION echo/printf is forbidden — and cerebrate emits none of those routing fields anyway.)
