...
**Mechanism 1 — spawn-time inject verification (spawn-brood).** After submitting the initial task paste, `spawn-brood` polls the child's run-ledger `state.current` for started-evidence within a bounded budget. On no started-evidence within that budget, it resends `send-keys Enter` up to a retry cap. On retry-cap exhaustion with still no started-evidence, the strain is recorded in the manifest with `status: "failed"` (failed-to-launch); `brood-status` renders it via the existing `failed`-precedence rule. This is NOT a new manifest field and `manifest_version` stays 4.
