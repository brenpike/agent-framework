...
| `trunk-freshness` | One of `fresh\|stale (N behind)\|skipped` — resolved during Required Git Preflight trunk freshness check; skipped when skip conditions from `${CLAUDE_PLUGIN_ROOT}/governance/branching-pr-workflow.md` (Trunk Freshness Gate) apply |
