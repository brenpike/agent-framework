...
| `local-review` | One of `active\|opted-out` — resolved at task intake by checking raw task input for opt-out keywords (`skip review`, `no review`, `skip local review`) per `${CLAUDE_PLUGIN_ROOT}/governance/execution-algorithm-detail.md` (Step 0) |
