...
- **Local-review opt-out detection (intake).** Check the user's raw task input for review opt-out keywords (case-insensitive substring match) BEFORE PR-feedback-remediation detection. Two tiers of opt-out are supported:
  - **Local-only opt-out:** "skip local review" → record `local-review: opted-out` and `review: active`. This suppresses local pre-PR review (Step 13a) only; post-PR review (Step 15) remains active.
  - **Global opt-out:** "skip review" or "no review" → record `local-review: opted-out` and `review: opted-out`. This suppresses both local pre-PR review (Step 13a) and post-PR review (Step 15).
