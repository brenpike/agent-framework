...
- **Local-review opt-out detection (intake).** Check the user's raw task input for review opt-out keywords (case-insensitive substring match). Two tiers of opt-out are supported:
  ...
  Match "skip local review" first (it is a substring of the broader "skip review" pattern). If no keyword is present, record `local-review: active` and `review: active` (defaults). These facts are resolved once and reused at every downstream routing decision involving review.
