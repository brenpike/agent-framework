...
- **Local-review opt-out detection (intake).** Check the user's raw task input for local-review opt-out keywords: "skip review", "no review", or "skip local review" (case-insensitive substring match). If any keyword is present, record `local-review: opted-out` in Session facts. Otherwise, record `local-review: active` (default). This fact is resolved once and reused at every downstream routing decision involving local review.
