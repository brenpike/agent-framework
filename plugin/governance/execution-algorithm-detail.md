...
- **Local-review opt-out detection (intake).** Check the user's raw task input for review opt-out keywords (case-insensitive substring match) BEFORE PR-feedback-remediation detection. Two tiers of opt-out are supported:
  - **Local-only opt-out:** "skip local review" → record `local-review: opted-out` and `review: active`. This suppresses local pre-PR review (Step 14) only; post-PR review (Step 16) remains active.
  - **Global opt-out:** "skip review" or "no review" → record `local-review: opted-out` and `review: opted-out`. This suppresses both local pre-PR review (Step 14) and post-PR review (Step 16).
  Match "skip local review" first (it is a substring of the broader "skip review" pattern). If no keyword is present, record `local-review: active` and `review: active` (defaults). These facts are resolved once and reused at every downstream routing decision involving review. After recording opt-out facts, produce a normalized copy of the raw input for the next sub-step by removing the matched opt-out phrase ("skip local review", "skip review", or "no review") from the text; if no opt-out matched, the normalized copy equals the raw input.
- **PR-feedback-remediation detection (intake).** Before claude-mem detection, check the **normalized** task input (with opt-out phrases removed) for a PR feedback remediation request. A match requires an action-plus-target pair — an action verb (`address`, `fix`, `resolve`, `handle`, `remediate`, `watch`, `monitor`, `wait`, `poll`, `loop`) combined with a feedback-target noun (`comments`, `feedback`, `threads`, `review comments`, `review feedback`, `review threads`, `findings`, `checks`, `CI`, `status checks`, `check status`, `CI status`). The bare noun `status` alone does not match — it must appear as part of one of the qualified phrases `status checks`, `check status`, or `CI status` to avoid false positives with non-review uses of the word (e.g., "fix the status badge"). The bare noun `review` alone does not match — it must appear as part of one of the qualified phrases `review comments`, `review feedback`, or `review threads` to avoid false positives with non-feedback uses of the word (e.g., "fix the review page"). PR number resolution follows two paths:
  - **Explicit PR reference:** The user provides `PR <N>`, `#<N>`, or a GitHub PR URL. Record `pr: <N>` directly.
  - **Branch-implicit resolution (fallback):** The action+target pair is present but no explicit PR reference is found. Resolve from the current branch: `gh pr list --head $(git branch --show-current) --state open --json number --jq '.[0].number'`. If the query returns a PR number, record `pr: <N>`. If no open PR is found for the current branch, record `pr: absent`.
  When a match succeeds (action+target pair present): record `routing: pr-feedback-remediation` in Session facts. Record `pr: <N>` when resolved by either path; record `pr: absent` when neither path yields a PR number. Per `${CLAUDE_PLUGIN_ROOT}/governance/agent-system-policy.md` (One-time vs watch routing): missing PR identifiers do not exclude the agent at routing time — PR identification is the agent's responsibility. Task-type classification still runs (defaults to `bugfix` per tie-break rule). The `routing` signal takes precedence over normal STT routing at `after=intake-complete`. Note: "fix PR 95" alone (action verb without feedback-target noun) does NOT match — it could mean "continue implementing PR 95." The heuristic requires the action+target pair to avoid false positives.
  - **Target extraction (within PR-feedback-remediation detection).** When `routing: pr-feedback-remediation` is detected, additionally scan the user's raw task input for a specific comment or thread reference. A match is any of: a GitHub comment URL (`github.com/.../pull/N#issuecomment-...` or `github.com/.../pull/N#discussion_r...`), a `pullrequestreview` URL, or an explicit comment/thread ID. When found: record `target: <comment URL or ID>` in Session facts. When not found: omit `target` (the github-reviewer treats absent target as all unresolved threads).
...
## Step 14: Pre-PR Local Review

**Pre-PR local review.** After validation and before pushing or opening a PR, invoke the `agent-framework:local-reviewer` agent when ALL of the following are true: (a) local-review: active in Session facts (resolved at intake); (b) codex-plugin-cc availability is unknown or confirmed present (the agent itself detects availability).
...
c. Handle terminal return per STT rows 44-54:
   - `exit: clean, no fix commits` → proceed to step 15 (open PR).
   ...
   - `blocked: codex unavailable` → log `Review: local pre-PR review skipped (codex unavailable)`, proceed to step 15.
   - `blocked: delegation-failed` (stage: fix-delegation, with actionable findings) → orchestrator takes over: delegate planner (opus) for remediation plan covering blocked findings → delegate per plan (coder or designer, opus) to implement → verify → validate → checkpoint-commit → re-invoke local-reviewer with `resume_from_ledger`.
   - `blocked: other` (not fix-delegation stage, or no actionable findings) → STOP: surface blocker.

d. Tool-error recovery (STT row 72): if the local-reviewer agent crashes or times out (exit 124/137) AFTER starting execution (fix ledger exists at `.agent-framework/review-loop/fix-ledger.yaml`), read the ledger path and re-invoke with `resume_from_ledger`. If no ledger exists (spawn failure), fall through to generic tool-error rows.

## Step 16: Post-PR Review

**Post-PR review.** After the PR is opened (step 15), check the `review` session fact first. If `review: opted-out` (set by the review opt-out detection at intake): skip review and proceed to the Final Report with `Review: Requested: no (opted out)`. Otherwise, invoke the `agent-framework:github-reviewer` agent when (a) the user request contains `review`, `codex`, or `audit`; OR (b) `CLAUDE.md` sets review-on-PR = true. If neither (a) nor (b) is true, skip review and proceed to the Final Report with `Review: Requested: no`. External review is opt-in; this is the default path.

### Standard Path: Mode Routing

When review is opted-in after PR open, route by keyword presence in the original user request:

- **STT row 55 — watch mode:** user request contains `watch`, `monitor`, `wait`, `poll`, or `loop` → invoke `agent-framework:github-reviewer` in watch mode:
...
- **STT row 56 — fix mode:** user request contains none of the watch keywords → invoke `agent-framework:github-reviewer` in fix mode (one-shot):
...
Handle terminal return per STT rows 60-68:
...
- On `succeeded, watch requested` (STT row 69): invoke github-reviewer in watch mode (same as standard path above).
- On `succeeded, no watch` (STT row 70): Final Report with `Review: Requested: yes`.
- On `blocked` (STT row 71): STOP: surface blocker.

### Tool-Error Recovery (STT row 73)
