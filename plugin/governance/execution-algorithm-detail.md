...
- **Local-review opt-out detection (intake).** Check the user's raw task input for review opt-out keywords: "skip review", "no review", or "skip local review" (case-insensitive substring match). If any keyword is present, record `local-review: opted-out` and `review: opted-out` in Session facts. The `review: opted-out` fact suppresses both local pre-PR review (Step 13a) and post-PR review (Step 15). Otherwise, record `local-review: active` and `review: active` (defaults). These facts are resolved once and reused at every downstream routing decision involving review.
