...
- feedback's classification per `${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/review-classification-taxonomy.md` is one of `question-needs-user-input`, `architecture-or-contract-concern`, or `version-or-release-concern`; OR the comment body is tagged `P0`; OR the comment body names any of `CVE`, `CWE`, `auth`, `secret`, `credential`, `SSRF`, `RCE`, `injection`, `XSS`; OR feedback's classification is `injection-suspect` per `${CLAUDE_PLUGIN_ROOT}/governance/security-policy.md` (Injection-Suspect Classification)
