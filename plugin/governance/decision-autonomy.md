...
- a PR is DERIVABLE for the run from a recorded event output — the PR persisted in the `open_pr` event's `outputs` (the overlord records the PR url into `event.outputs.pr` when recording the `open_pr` state, from the `url` `hivemind:open-plan-pr` returns) for a standard-delivery run. A `pr-feedback-remediation` run watches/fixes an EXISTING PR and journals Tier-B decisions but never emits `open_pr`, so its PR is NOT currently derivable here — no sanctioned writer persists the remediation PR (init seeds `facts.pr` null and `hivemind:record-state-result` never mutates `facts.*`). Post-merge reporting for `pr-feedback-remediation` runs therefore requires persisting the resolved PR for those runs; that work is tracked in issue #302 and is NOT yet wired
