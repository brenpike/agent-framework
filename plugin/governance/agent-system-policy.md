...
See `${CLAUDE_PLUGIN_ROOT}/governance/core-contract.md` (Mandatory Modules) for the canonical list. Mandatory modules are always loaded; no activation condition, user override, or workflow classification can suppress them.

## Conditional Governance Files

See `${CLAUDE_PLUGIN_ROOT}/governance/core-contract.md` (Conditional Modules) for canonical activation conditions. When uncertain whether a condition is met, include the module (fail-open).
...
The `general-purpose` entry in each agent's `Agent(...)` tool scope exists exclusively so that skills and reviewer agents can transitively invoke helper subagents for narrow classification and analysis tasks. The following uses of `general-purpose` agents are explicitly prohibited: planning, implementation, design, fallback routing, and companion-agent delegation — unless a separate policy explicitly authorizes that path. The orchestrator must not invoke `general-purpose` agents from its own body directly; `general-purpose` agent calls must only originate from within skills or reviewer agents. Reviewer agents (`local-reviewer`, `github-reviewer`) additionally use `Agent` to delegate simple fixes to `coder` and `designer` at sonnet tier — this is authorized by their agent definitions and the Authority Matrix.

External reviewers, CI, GitHub, Codex, and other services are not Claude Code subagents.

**Skill subagent pattern**: Skills may spawn general-purpose subagents (`general-purpose` agent, no `subagent_type`) using custom instruction `.md` files for narrow classification and analysis tasks (e.g. injection-suspect checking, feedback classification, thread resolution decisions). These are NOT framework agents and must not be listed in the Allowed Agent Topology. Only the orchestrator and reviewer agents may delegate to framework agents — skills must return classification results and routing recommendations to the orchestrator, which owns all framework agent delegation (reviewer agents may delegate simple fixes to coder/designer at sonnet tier without orchestrator mediation).
...
| claude-mem | optional | receive `Memory context:` from orchestrator's pre-planning lookup; invoke `claude-mem:mem-search` directly only as fallback when `Memory context:` field is absent from delegation | use when prior decisions about the same files exist in memory | same condition as coder | continuity and token efficiency |
