...
1. **Fix and continue** — update the local trunk ref using the checkout-safe approach for the current branch state, then re-check divergence to confirm `0\t0`:
   ...
   - When LEFT>0 (local trunk has unpushed commits): the local trunk has diverged from origin. Present user with additional sub-choice: push the local commits (`git push origin <trunk>`) or reset local trunk to match origin — when not on trunk: run the worktree pre-check first (same command as above); if clear, use `git branch -f <trunk> origin/<trunk>` (force-sets the local branch pointer without checkout); when on trunk: `git reset --keep origin/<trunk>` (resets to origin but aborts if local changes would be overwritten — if it exits non-zero, stash or commit local changes first, then retry). Confirm `0\t0` after the chosen action before proceeding.
