...
1. **Fix and continue** — update the local trunk ref using the checkout-safe approach for the current branch state, then re-check divergence to confirm `0\t0`:
   - **Worktree pre-check (required):** run `git worktree list --porcelain | grep -q "^branch refs/heads/<trunk>$"` to detect whether `<trunk>` is checked out in another linked worktree. If detected (exit 0): block — do not attempt to update the trunk ref from the feature worktree. Surface a message instructing the user to switch to the trunk worktree and run `git pull --ff-only origin <trunk>` there, then return here and re-run preflight.
   - When currently on trunk: `git pull --ff-only origin <trunk>` (safe while trunk is checked out).
   - When currently on a working branch: `git fetch origin <trunk>:<trunk>` (updates the local trunk ref directly without switching; safe when trunk is not checked out in another worktree, as confirmed by the pre-check above). If the local trunk cannot be fast-forwarded, git returns a non-zero exit; treat that as a blocker and report to user.
