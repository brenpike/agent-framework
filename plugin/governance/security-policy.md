...
3. **hostile `--dangerously-skip-permissions` child → coordinator (#161, CLOSED; read anchor re-grounded in #168; current-worktree anchoring in #182).** The coordinator's read of untrusted cross-worktree child-ledger content is under this discipline via `brood-status-project.sh`: manifest-path identifier input, GROUND-TRUTH path derivation (the per-strain ledger anchor is the REAL worktree from `git worktree list --porcelain` keyed by the manifest branch — the branch is a lookup key only, never a path; the manifest `worktree_path` is display-only, NOT an anchor), confinement chain `CHECKOUT_ROOT ⊇ git-worktree ⊇ ledger` via `_shared/containment.sh`, two-scalar bounded jq projection (run.status enum + state.current charset/length ≤64), MISSING/MALFORMED tokens, informational-only (never overrides observable status). The `CHECKOUT_ROOT` is now per-coordinator `git rev-parse --show-toplevel` (the CURRENT checkout the dashboard runs in), not a forced main-checkout resolution — the same anchor `spawn-brood.sh` writes against. The #161 guarantee rests on the DISCIPLINE above, which holds for ANY `CHECKOUT_ROOT`, so it applies at EVERY hatchery level: the main coordinator's ceiling is unchanged because it still runs from the main checkout (untouchable by a child), and a child-hatchery applies the SAME discipline one level down. Nested-brood visibility (#182) is the recursive application of this same boundary — no boundary is widened (spawn-brood already supports recursive spawn; #182 only makes an already-possible nested brood visible through the read path). The leaf symlink-swap micro-TOCTOU between the `[ -L ]` re-check and the `cat` is an ACCEPTED BOUNDED RESIDUAL — NOT structurally closed by per-brood namespacing (which isolates broods from each other, not the hatchery from its children, since the hatchery reads child ledgers by design): it is bounded by a post-read containment re-assert, never-echo-raw projection, and the informational-only contract (ADR-0021; ADR-0019 #168 amendment; #182 current-worktree-anchor clarification).
...
- `hivemind:spawn-brood` → `.hivemind/spawn-inputs.<rand>.json` (per-invocation mktemp-unique STAGING path under `.hivemind/`; the script `mv`s it into `.hivemind/broods/<brood-id>/inputs.json` after generating the GUID brood-id; per-brood-id namespacing dissolves the prior singleton-inputs/manifest TOCTOU and the liveness guard is removed — #168; see the invariant bullet above for full detail)
