...
The four navigators and the rationale for each transport:
...
- `hivemind:decision-report` → `.hivemind/runs/.decision-report-inputs-<token>.json` — fixed-literal `.hivemind/runs/` prefix + per-invocation `<token>`, authored as a SIBLING of the run dirs (no caller-derived component below the fixed level). The navigator RENDERS the post-merge decision narrative (the chat deliverable) and authors this inert inputs file (`run_id`, the rendered `report_markdown`, `pr_state`); `decision-report.sh` then DERIVES the report path `<git-root>/.hivemind/runs/<run_id>/decision-report.md` from `run_id` (SAFE_ID_RE-validated, `.`/`..` rejected) + the git root, runs the shared `hivemind_assert_file_contained` write-target LEAF guard on that resolved path (rejecting a symlinked `<run_id>` dir OR a symlinked `decision-report.md` leaf), and writes the rendered narrative atomically via temp-file + `mv`. Before this engine the skill wrote the report with the agent Write tool directly to `.hivemind/runs/<run_id>/decision-report.md` — a caller-derived `<run_id>` BELOW the fixed level, which a committed symlinked `<run_id>` dir or `decision-report.md` leaf could redirect outside the checkout BEFORE any inline check ran (the agent Write fires first; an inline realpath check is UNSOUND). Demoting the agent Write to this fixed-literal inert inputs file and routing the report write through the containment-checked engine closes that F1 P0 vector. No ledger marker is written — the report file's existence is the sole idempotency marker.
...
The four covered skills and their fixed-literal transport paths (per the transport-path invariant above):

- `hivemind:spawn-brood` → `.hivemind/spawn-inputs.<rand>.json` (per-invocation mktemp-unique STAGING path under `.hivemind/`; the script `mv`s it into `.hivemind/broods/<brood-id>/inputs.json` after generating the GUID brood-id; per-brood-id namespacing dissolves the prior singleton-inputs/manifest TOCTOU and the liveness guard is removed; see the invariant bullet above for full detail)
