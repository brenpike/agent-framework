...
- `hivemind:spawn-brood` → `.hivemind/brood/inputs.json` — fixed-literal singleton. `brood` is a constant, NOT a caller-derived component, so the no-caller-derived-component rule holds without a token. It is NOT covered by the per-invocation-`<token>` requirement and is documented as a KNOWN v1 exception pending #168: the ADR-0017 liveness guard is a check-then-act read with a TOCTOU window — it is NOT a reservation and does NOT actually serialize concurrent same-checkout spawns. Two concurrent same-checkout spawns can both pass the liveness check before either writes a manifest, both launch `--dangerously-skip-permissions` children, and the later manifest write hides the earlier child from monitoring; the singleton `inputs.json` is likewise clobberable between the navigator Write and the script exec. ROOT CAUSE: the v1 SINGLETON shared-manifest layout (the manifest is the sole shared mutable artifact per RUN-OWNERSHIP-01). The STRUCTURAL fix is per-`<brood-id>` namespacing (`.hivemind/broods/<brood-id>/...`), tracked in #168, which gives each brood its own disjoint dir/manifest/inputs and dissolves both races; a per-checkout lock was deliberately NOT added (throwaway once namespacing lands). Do not add a token or lock to the singleton transport as a substitute — the singleton layout itself is the residual, and #168 removes the sharing rather than locking it.
...
- `hivemind:spawn-brood` → `.hivemind/brood/inputs.json` (fixed-literal singleton — `brood` is a constant, no caller-derived component; NOT covered by the per-invocation `<token>` requirement; KNOWN v1 residual TOCTOU — the liveness guard is a check-then-act read, not a reservation; structural fix is per-`<brood-id>` namespacing tracked in #168; see the invariant bullet above for full detail)
