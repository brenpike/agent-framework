...
4. **Recovery Action — re-delegate the failing phase.** When the four rehydration sources above do not recover the missing fields, re-invoke the worker for the failing phase with explicit instructions to produce the missing fields. This is a worker invocation, not a retrieval source, and is the last step before escalation. The worker report from this re-invocation is the authoritative source for fields produced in the current phase.
5. (Optional) When persistence of the in-progress, failed candidate is required for diagnostics, write a **quarantined draft artifact** to `.agent-framework/checkpoints/STEP-NNN-quarantine.md` (or `TASK-NNN-quarantine.md` for STEP-bypass tasks). Quarantine drafts are diagnostic only; they are not handoffs and must not be consumed by the next phase or by Path B rehydration.
6. After rehydration (and re-delegation, if performed), merge the recovered fields into the held candidate handoff and re-run the reconstruction test.
