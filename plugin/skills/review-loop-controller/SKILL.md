...
5. **Check exit conditions before running review.**
   - If iteration count >= `max_iterations`: persist updated ledger to storage per the step 15 persistence mechanism (including any fix_commit entries recorded in step 4), then return with `exit_reason: "max-iterations-reached"`, current ledger state, and all open findings (see Per-Iteration Result Contract).
...
8. **Handle approve verdict.**
   - If `verdict: "approve"` and `findings` is empty: set `exit_reason: "clean"`, persist updated ledger to storage per the step 15 persistence mechanism (including any fix_commit entries recorded in step 4), return.
   - If `verdict: "approve"` and `findings` is non-empty: surface informational findings in output, set `exit_reason: "clean"` (verdict drives exit, not finding count), persist updated ledger to storage per the step 15 persistence mechanism (including any fix_commit entries recorded in step 4), return.
...
12. **All-non-actionable check.** If every finding in the current iteration classifies as `non-actionable` or `incorrect-or-rejected` (zero actionable findings remain after classification): set `exit_reason: "clean"`, record in ledger, persist updated ledger to storage per the step 15 persistence mechanism before returning, return. Re-running would review the same unchanged diff and reproduce the same result.
