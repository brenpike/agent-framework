...
12. **User-input-required check.** If any finding in the current iteration classifies as `question-needs-user-input`: set `exit_reason: "user-input-required"` in the ledger; `printf 'exit_reason: user-input-required\nfinding_id: %s\nbody_excerpt: %.200s\n' "$finding_id" "$body"; exit 0`. This check runs before break-fix detection and the normal iteration result — `question-needs-user-input` is a terminal exit condition that must not be masked by `routing: none` mapping.
