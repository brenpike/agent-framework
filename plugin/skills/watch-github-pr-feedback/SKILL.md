...
- `OWNER` and `REPO` from `gh pr view PR_NUMBER --json url --jq '.url | ltrimstr("https://github.com/") | split("/") | .[0] + " " + .[1]'` (split on space; `Bash(gh pr view *)` is already in the skill's allowed tools)
