...
6. **Identify the target and route.** Apply routing rules in order — first matching rule wins:
   ...
   - **User-named target:** if the user named a specific target (by URL, comment ID, review ID, or quoted text) and it exists in the candidate set, route that target based on its classification: if `architecture-or-contract-concern` or `version-or-release-concern`, or the smallest correct fix would touch files in more than one planner step, route to `agent-framework:planner`; if `actionable-code-change`, `actionable-test-change`, or `actionable-doc-change`, route to `agent-framework:coder`; if `design-or-UX-concern`, route to `agent-framework:designer`; if `incorrect-or-rejected`, follow the Incorrect/rejected rules below; if `non-actionable`, treat as Nothing actionable. Skip the remaining ordering and disambiguation rules.
