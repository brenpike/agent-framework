...
6. **Return classification result to orchestrator.** Do not delegate to framework agents. Apply routing rules in order — first matching rule wins — and return the structured result:
   ...
   - **Cross-step planner routing:** if any candidate classifies as `actionable-*` AND its Smallest correct fix would touch files in more than one planner step, route to `planner` regardless of whether one candidate is user-named and regardless of how many candidates exist. This rule fires before user-named-target routing and before disambiguation to prevent planner-class items from being bypassed by a named-target shortcut or blocked at disambiguation.
   ...
   **Cross-step reminder:** Multi-step fix routing is handled by the ordering rule above (before user-named-target routing and disambiguation). The routing table below applies after that gate has passed.
