...
6. **Return classification result to orchestrator.** Do not delegate to framework agents. Apply routing rules in order — first matching rule wins — and return the structured result:
   ...
   - **Single actionable candidate:** if exactly one candidate classifies as any actionable or design classification, determine routing based on its classification (see routing table below).
...
2. **Post fix-SHA reply.** Post a reply on the feedback thread with the fix SHA. Use the GraphQL `addComment` mutation (see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/github-pr-review-graphql.md`). If `validated: no`, note in the reply that validation did not pass. Reply mechanism by feedback source:
