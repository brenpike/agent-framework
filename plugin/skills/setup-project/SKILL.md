...
14. Detect the project's test command and record it under a `## Validation` section in the repo-root `CLAUDE.md` if absent. DETECT only — never run, install, or scaffold a harness. This is the command `hivemind:refactor-to-depth` reads as its green-gate. Each signal must resolve to a real, executable test command — skip placeholder, no-op, or known-failing scripts; never record a command that would fail the green-gate on first run.
    a. Match each signal below against repo-root or conventional well-known locations. Require the ACTUAL signal, not mere file existence — inspect the file content named in the signal:
       - `package.json` with a `scripts.test` entry whose value is a string that does NOT contain the case-insensitive substring `Error: no test specified` → that script (`npm test`). When the value DOES contain that substring (the `npm init` / `yarn init` / `pnpm init` placeholder `"echo \"Error: no test specified\" && exit 1"`), treat this signal as UNMATCHED and fall through to the next sub-bullets (vitest dep/config; jest dep/config). The content-based substring match is intentionally runner-agnostic — it auto-covers all three init defaults; do not add per-runner branches. Non-string `scripts.test` values (array, object) are likewise treated as unmatched and fall through. Missing `scripts` object entirely is already unmatched and unchanged. Accepted tradeoff: chained scripts that mention the placeholder string mid-pipeline (e.g. `"echo \"Error: no test specified\" && vitest run"`) are also rejected by this substring match; the vitest/jest fall-through below covers the common recovery, and if neither matches the JS ecosystem contributes nothing and the existing no-signal path applies.
