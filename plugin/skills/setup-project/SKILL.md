...
5. Merge required keys, preserving every existing key the user already had:
   ...
   - if `seed_allowlist` = `yes`: merge the recommended least-privilege template below into `permissions.allow` using union/append-if-absent semantics (see Merge Rules) — append only rules not already present, preserving the user's existing entries and their order. The frozen template rule set (mirror exactly; do not add, drop, or reword entries):
     ...
     ```
     The template grants read-helper Bash commands, scoped git read subcommands (`git tag` is list-only via the three `git tag` / `git tag -l*` / `git tag --list*` forms — never broad `git tag *`, which would permit creating tags), and the codex-companion node entry. `printf` routing rules are intentionally NOT seeded: a `Bash(printf 'X: *)` rule permits a redirect-write tail (`printf 'X: ...' > file`), an unacceptable auto-approved file-write surface. Pipeline skills (`molt`, `create-working-branch`, `open-plan-pr`, `adaptation-cycle`) currently emit routing data via their own `allowed-tools` `Bash(printf *)` grants — those grants carry the same redirect-write tail and represent a known surface; tightening them is tracked as a separate hardening effort. Project-level `printf` is not seeded because the concern applies equally there: any `Bash(printf 'X: *)` grant permits a redirect-write tail (`printf 'X: ...' > file`) as an auto-approved file-write surface. Broad `Bash(echo *)`, `Bash(printf *)`, and `Bash(cat *)` are intentionally NOT seeded for the same reason. The codex-companion rule uses the literal placeholder path `/path/to/.claude/plugins/cache/openai-codex/codex/*`; the consumer must replace `/path/to/` with their own home directory path after setup. Do NOT seed `acceptEdits` mode, `Edit`, or `Write` into consumer settings.
6. If `dry_run` = `yes`:
