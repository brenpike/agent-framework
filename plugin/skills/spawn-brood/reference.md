...
Security rationale, manifest discipline, dependency notes, and the authoritative
inputs schema for `hivemind:spawn-brood`. The skill body (`SKILL.md`) is a
navigator; the deterministic engine is
`${CLAUDE_PLUGIN_ROOT}/skills/spawn-brood/scripts/spawn-brood.sh`. This file holds
the WHY that the navigator omits.

## Contents

- [Inputs JSON schema (authoritative)](#inputs-json-schema-authoritative)
...
## Inputs JSON schema (authoritative)

The agent authors ONE JSON file via the Write tool at
`.hivemind/brood/inputs.json` (in the coordinator checkout; `.hivemind/` is
gitignored), then passes its path as the script's single positional argument. The
script parses it with `jq` into shell variables. Exact shape:
...
- `strains` MUST be a non-empty array. A zero-strain array is a blocker (the
  script never writes an empty manifest).
- `brood_id`, `base`, `overlap_risk` are required top-level scalars.
- Each strain requires `name`, `description`, `branch`.
- `name` is sanitized by the script to `short` (`[a-z0-9-]`, lowercase) for the
  worktree path (`.claude/worktrees/<short>`) and tmux session (`brood-<short>`).
  Two distinct names that sanitize to the same `short` are an in-set collision and
  a blocker.
