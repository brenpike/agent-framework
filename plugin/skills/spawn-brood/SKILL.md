...
The cerebrate resolves and passes these. The skill does not resolve them on its own.

- `streams`: array of `{name, description, branch}` — each strain to spawn.
- `fleet_id`: ISO-8601 timestamp (generated by cerebrate).
...
- Exit 0 = cerebrate proceeds. Routing data (if any) is in stdout.
