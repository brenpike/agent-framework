...
1. **Build the inputs object** in your reasoning from the five Required Inputs:
   a JSON object with top-level `brood_id`, `base`, `overlap_risk`,
   `overlap_details`, and a `strains` array of `{name, description, branch}`. The
   authoritative schema is in
   `${CLAUDE_PLUGIN_ROOT}/skills/spawn-brood/reference.md` (Inputs JSON schema).
...
- Security rationale, manifest discipline, and the inputs schema:
