...
3. Identify the target item. The candidate set is the union of:
   ...
   - **Planner-class feedback**: if at least one candidate classifies as `architecture-or-contract-concern` or `version-or-release-concern`, OR the Smallest correct fix for any candidate would touch more than one file in different planner steps (per `${CLAUDE_PLUGIN_ROOT}/governance/pr-review-remediation-loop.md` Routing), route to `agent-framework:planner`. Do not delegate to coder/designer until the planner returns a plan.
