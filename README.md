...
The seeded entries cover read-helper Bash commands (e.g. `grep`, `find`, `cat`), scoped git read subcommands, and `echo`/`printf`. They are appended-if-absent — existing entries are never overwritten or removed. `node`, `Edit`, and `Write` are intentionally **not** auto-approved; those require explicit per-project decisions.
