...
- **Standing rule.** No local-review provider grant — `codex`, `copilot`, or any future provider — may be seeded into the committed `.claude/settings.json`. Three reasons: (a) that file is committed and team-shared, so a machine-specific `$HOME` cache path is wrong for everyone but the seeder; (b) `hivemind:adaptation-cycle`'s SKILL.md `allowed-tools` carries `Bash(node *)`, which PRE-APPROVES the CODEX invocation (codex is invoked via node), per the **Amendment — 2026-07-27** engine fact that `allowed-tools` pre-approves a permission but never provisions a tool — and a future or non-default provider's binary grant (for example `Bash(copilot *)`) belongs in that SAME skill frontmatter list, NOT in the seeded template; (c) per-contributor grants belong in the gitignored `.claude/settings.local.json`, which is what `.devcontainer/postCreate.sh` already does.
  - **Known gap (2026-08-24).** As of this amendment `Bash(copilot *)` is NOT present in that frontmatter, so a copilot-provider review will hit an interactive permission prompt until it is added.
- **This REINFORCES Decision #3.** Decision #3 excludes `Bash(node *)` from the seeded allowlist for arbitrary-code-execution reasons. Removing a provider-specific `node` grant from the seed keeps that exclusion whole; the skill-scoped `allowed-tools` pre-approval is a narrower surface than a project-wide allow rule, not a contradiction of it.
