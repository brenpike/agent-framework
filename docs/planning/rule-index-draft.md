...
| Validation gate | `VAL-01` | `plugin/governance/agent-system-policy.md` (Definitions -- Validation procedure) | orchestrator | orchestrator, coder, designer, open-plan-pr, checkpoint-commit, address-github-pr-feedback, watch-github-pr-feedback | planned | Validation must run every declared command. Cannot skip silently. PR and merge readiness depend on validation passing or being explicitly reported. |
